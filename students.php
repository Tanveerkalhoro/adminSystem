...
if(isset($mode) && $mode=="delete" && isset($id)){
    $sql4     = "DELETE  FROM student_info WHERE student_id='$id' ";
...
if(isset($mode) && $mode=="edit" && isset($id)){
    $sql2     = "SELECT * FROM student_info WHERE student_id='$id' ";
...
if(isset($is_submit) && $is_submit=="Y"){
    ...
    if(empty($error)){
        if($mode=="edit"){
            ...
            $sql2     = "SELECT * FROM student_info WHERE user_name='$user_name' AND full_name='$full_name' AND email='$email' ";
            $result2  =mysqli_query($conn,$sql2);
            $count2   =mysqli_num_rows($result2);
            if($count2 > 0) {
            $error = "Sorry ! This record is already exist.";
            } else {
                $sql1  ="INSERT INTO student_info(full_name,user_name,email,pasword,role_id) values('$full_name','$user_name','$email','$pasword','$role_id')";
                $result1 =mysqli_query($conn,$sql1);
                if($result1 > 0){
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="students.php?menu_id=<?php echo $menu_id;?>&mode=edit&id=<?php echo $id?>">Edit</a>  &nbsp;&nbsp;
                        <a href="students.php?menu_id=<?php echo $menu_id;?>&mode=delete&id=<?php echo $id?>">Delete</a>
                    </td>                   
                    <td> 
                        <?php 
